...
import { getFcClient } from '@alicloud/fun/lib/client';
export class AliyunFCPlugin extends BasePlugin {
  ...
  hooks = {
    ...
    'deploy:deploy': async () => {
      ...
      console.log(join(this.midwayBuildPath, 'template.yml'));

      const fc = getFcClient();
      try {
        const service = await fc.getService('serverless-hello-world');
        this.core.cli.log('=====');
        this.core.cli.log(service);
        this.core.cli.log(JSON.stringify(service));
