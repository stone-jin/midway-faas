...
const deploy = async () => {
  const result = await request('/api/product/deploy', {
    method: 'POST',
    data: {
      filePath: '/Users/hzjinbing/code/test/juhai2',
    },
  });
  console.log(result);
};
...
class Info extends Component<
  { loading: boolean; productAndinfo: AdvancedProfileData; dispatch: Dispatch<any> },
  InfoState
> {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'productAndinfo/fetchAdvanced',
    });
  }
  ...
  render() {
    ...
    const { productAndinfo, loading } = this.props;
    const { advancedOperation1, advancedOperation2, advancedOperation3 } = productAndinfo;
    const contentList = {
      tab1: (
        <Table
          pagination={false}
...
export default connect(
  ({
    ...
    productAndinfo: AdvancedProfileData;
    loading: {
      effects: { [key: string]: boolean };
    };
  }) => ({
    productAndinfo,
    loading: loading.effects['productAndinfo/fetchAdvanced'],
  }),
)(Info);
